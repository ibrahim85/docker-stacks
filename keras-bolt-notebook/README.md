...
# Jupyter Notebook Python, Bolt, Spark, Keras, Elephas Stack
...
* bolt-python for distributed nd-arrays
* TensorFlow 0.9, Keras 1.0.8, Elephas 0.3 for Deep Learning
* Spark 2.0.1 for use in local mode or to connect to a cluster of Spark workers
...
## Building
The build takes quite a while to run the first time since many packages need to be downloaded and installed

```
docker build ./ -t fq/mlbolt
```
...
```
docker run -d -p 8899:8888 fq/mlbolt
```
